@doc = """
  ...
  A framer is a `Stage(String, Result(String, FramingError))`. In Zap a
  ...
  out as `Result.Error(%FramingError{...})` followed immediately by `:halt` —
...
pub struct Framer {
  @doc = """
    ...
    emits `Result.Error(%FramingError{reason: :oversize})` and halts; leftover
    bytes at end-of-stream emit `Result.Error(%FramingError{reason:
    :truncated})`.
  ...
  pub fn length_prefixed(prefix_bytes :: i64, max_frame_size :: i64) -> LengthPrefixedFramer {
    ...
    %LengthPrefixedFramer{prefix_bytes: checked_prefix_bytes, max_frame_size: checked_max_frame_size, buffer: ""}
  }
  ...
  @doc = """
    ...
    `max_frame_size` bytes emits `Result.Error(%FramingError{reason:
  ...
  pub fn line(max_frame_size :: i64) -> LineFramer {
    checked_max_frame_size = Framer.check_max_frame_size(max_frame_size)
    %LineFramer{max_frame_size: checked_max_frame_size, buffer: ""}
  }
