...
**STRICTLY FORBIDDEN.** Never duplicate APIs as a first resort. If you are writing code and think that you need to new a new function, struct, or any new work you must first see if existing functionality exists that you can use. Or if existing functionality can and should be extended responsibly without bloating the single responsibility principal for functionality. If conditions cannot be met only then can you write new functions, structs, etc...
