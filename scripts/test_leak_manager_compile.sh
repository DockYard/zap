#!/bin/bash
# Real-Zig smoke test for the production Leak memory-manager source.
#
# Compiles `src/memory/leak/manager.zig` with the system `zig` compiler
...
#   4. The section's `declared_caps` field is **0** — Leak declares no
#      capabilities (Phase 7 ships a CI diagnostic manager whose entire
#      purpose is to verify that Phase 6's codegen elision actually
#      produces zero refcount call sites when capabilities are absent).
#      This matches the Arena manager's declared_caps signature; the
#      difference is intent — Leak never frees, Arena bulk-frees at
#      deinit.
#
# This is the Phase 7 Leak counterpart of `test_arena_manager_compile.sh`
# (Phase 5 Arena), `test_arc_manager_compile.sh` (Phase 4 ARC), and
# `test_manager_compile.sh` (Phase 3 NoOp). The Phase 7 integration tests
# in `src/memory/driver.zig` ("Phase 7 integration: Leak manager resolves
# end-to-end") use a synthesized object whose section layout mimics this
# manager but does NOT compile the actual source. The script below
# closes that gap by exercising the real source through the real system
# `zig`, catching drift between what we think the manager emits and what
# the toolchain actually produces.
#
# Usage:
#   ./scripts/test_leak_manager_compile.sh
#
# Exit codes:
...
MANAGER_SRC="$PROJECT_ROOT/src/memory/leak/manager.zig"
...
PROBE_SRC="$PROJECT_ROOT/src/memory/__leak_driver_check_probe.zig"
...
pub fn main(init: std.process.Init) !void {
    ...
    driver.assertExportsManagerSymbolForTest("real_leak", bytes, &diag) catch |err| {
...
#
# Leak declares no capabilities. Read the section payload and assert
...
echo ">>> Verifying declared_caps == 0 (Leak declares no capabilities)..."
...
if extract_section; then
    ...
    # in `declared_caps` would mean the Leak manager is advertising a
    ...
    if [ "$DECLARED_CAPS_HEX" = "0000000000000000" ]; then
        ...
        echo "       Expected the Leak manager to declare zero capabilities." >&2
        exit 1
    fi
...
echo ""
echo ">>> Leak manager compilation smoke test PASSED."
