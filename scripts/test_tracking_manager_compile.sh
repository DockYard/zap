#!/bin/bash
# Real-Zig smoke test for the production Tracking memory-manager source.
#
# Compiles `src/memory/tracking/manager.zig` with the system `zig` compiler
...
# `test_arc_manager_compile.sh` (Phase 4 ARC), and
# `test_manager_compile.sh` (Phase 3 NoOp). The Phase 7 integration tests
# in `src/memory/driver.zig` ("Phase 7 integration: Tracking manager
# resolves end-to-end") use a synthesized object whose section layout
# mimics this manager but does NOT compile the actual source. The script
# below closes that gap by exercising the real source through the real
# system `zig`, catching drift between what we think the manager emits
# and what the toolchain actually produces.
#
# Usage:
...
MANAGER_SRC="$PROJECT_ROOT/src/memory/tracking/manager.zig"
...
PROBE_SRC="$PROJECT_ROOT/src/memory/__tracking_driver_check_probe.zig"
...
pub fn main(init: std.process.Init) !void {
    ...
    driver.assertExportsManagerSymbolForTest("real_tracking", bytes, &diag) catch |err| {
...
#
# Tracking declares no capabilities. Read the section payload and
# assert the entire `declared_caps` u64 is zero. The meta header
# layout is:
#
...
echo ""
echo ">>> Tracking manager compilation smoke test PASSED."
