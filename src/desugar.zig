...
pub const Desugarer = struct {
    ...
    fn desugarModule(self: *Desugarer, mod: *const ast.ModuleDecl) !ast.ModuleDecl {
        ...
        if (self.graph) |g| {
            self.current_module_scope = g.findModuleScope(mod.name);
        }
    ...
    fn resolveImportedModule(self: *const Desugarer, name: ast.StringId, arity: u32) ?ast.ModuleName {
        const graph = self.graph orelse return null;
        ...
        for (scope.imports.items) |imp| {
            if (graph.findModuleScope(imp.source_module)) |imp_scope_id| {
