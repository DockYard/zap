...
pub const Desugarer = struct {
    ...
    for_counter: u32 = 0,
    /// Monotonic counter for eta-expanded function-reference wrappers
    /// (`etaExpandFunctionRefArg`). Every `&Struct.fn/arity` argument
    /// gets its own uniquely-named forwarding wrapper: a shared constant
    /// name would make every wrapper in a struct collide into one
    /// function-group name at HIR/IR level, silently collapsing all
    /// function-reference arguments in that struct onto whichever
    /// reference was processed last (a wrong-code bug, not a
    /// diagnostic).
    fnref_eta_counter: u32 = 0,
    /// Uniquifying counter for parser-synthesized anonymous-function names
    /// (`__anon_fn_N`). The parser's own counter is per-PARSE, so a MACRO
    /// whose quote body contains an anonymous function (e.g. `Task.async`'s
    /// worker wrapper in `lib/task.zap`) clones ONE parsed decl — and ONE
    /// name — into every expansion site. Without a per-expansion rename,
    /// every expansion in a struct emits an identically-named function and
    /// the ZIR builder's name-keyed deduplication silently fuses them —
    /// every `Task.async` in a file would spawn the FIRST expansion's
    /// worker (a wrong-code bug, not a diagnostic). Same discipline as
    /// `fnref_eta_counter` above.
    anon_fn_unique_counter: u32 = 0,
        ...
        switch (expr.*) {
            ...
            .anonymous_function => |anon| {
                ...
                return try self.create(ast.Expr, .{
                    .anonymous_function = .{
                        .meta = anon.meta,
                        .decl = try self.desugarFunctionDecl(try self.uniquifyAnonymousFunctionDecl(anon.decl)),
                    },
                });
            },
    ...
    fn etaExpandFunctionRefArg(self: *Desugarer, fr: ast.FunctionRefExpr) !*const ast.Expr {
        ...
        // The wrapper name must be UNIQUE PER REFERENCE (see
        // `fnref_eta_counter`): the referenced function's name is folded
        // in (identifier-safe characters only) for debuggability, and the
        // counter guarantees uniqueness even for operator references or
        // repeated references to the same target.
        const wrapper_name_text = blk: {
            var buffer: std.ArrayListUnmanaged(u8) = .empty;
            errdefer buffer.deinit(self.allocator);
            try buffer.appendSlice(self.allocator, "__fnref_eta_");
            try buffer.print(self.allocator, "{d}_", .{self.fnref_eta_counter});
            self.fnref_eta_counter += 1;
            for (self.interner.get(fr.function)) |character| {
                if (std.ascii.isAlphanumeric(character) or character == '_') {
                    try buffer.append(self.allocator, character);
                }
            }
            break :blk try buffer.toOwnedSlice(self.allocator);
        };
        defer self.allocator.free(wrapper_name_text);
        const decl = try self.create(ast.FunctionDecl, .{
            .meta = meta,
            .name = try self.interner.intern(wrapper_name_text),
    ...
    /// Re-name a parser-synthesized anonymous function (`__anon_fn_N`) with
    /// a desugar-unique suffix. The parser mints `__anon_fn_N` once per
    /// PARSE, so a macro whose quote contains an anonymous function clones
    /// the same decl — and the same name — into every expansion site; two
    /// expansions in one struct would then emit two functions under one
    /// symbol and the ZIR builder's name-keyed deduplication silently fuses
    /// them (see `anon_fn_unique_counter`). The rename keeps the
    /// `__anon_fn_` prefix (the anonymous-function recognizers key on it)
    /// and is safe because downstream references to the closure are by
    /// function-group ID; the name only becomes the emitted symbol. A decl
    /// whose name is not parser-synthesized (an eta wrapper, a named local
    /// fn) passes through untouched.
    fn uniquifyAnonymousFunctionDecl(self: *Desugarer, decl: *const ast.FunctionDecl) !*const ast.FunctionDecl {
        const original_name = self.interner.get(decl.name);
        if (!std.mem.startsWith(u8, original_name, "__anon_fn_")) return decl;
        const unique_name_text = try std.fmt.allocPrint(
            self.allocator,
            "{s}__x{d}",
            .{ original_name, self.anon_fn_unique_counter },
        );
        defer self.allocator.free(unique_name_text);
        self.anon_fn_unique_counter += 1;
        return try self.create(ast.FunctionDecl, .{
            .meta = decl.meta,
            .name = try self.interner.intern(unique_name_text),
            .name_expr = decl.name_expr,
