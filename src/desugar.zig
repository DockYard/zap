        ...
        switch (expr.*) {
            ...
            .binary_op => |bo| {
                ...
                if (bo.op == .in_op) {
                    ...
                    const mod_ref = try self.create(ast.Expr, .{
                        .module_ref = .{ .meta = bo.meta, .name = .{ .parts = parts, .span = bo.meta.span } },
                    });
            ...
            .map => |me| {
                if (me.update_source) |source| {
                    ...
                    for (me.fields) |field| {
                        ...
                        parts[0] = proto_name;
                        const mod_ref = try self.create(ast.Expr, .{
                            .module_ref = .{ .meta = me.meta, .name = .{ .parts = parts, .span = me.meta.span } },
                        });
                        const callee = try self.create(ast.Expr, .{
            ...
            .range => |re| {
                ...
                return self.create(ast.Expr, .{
                    .struct_expr = .{
                        .meta = re.meta,
                        .module_name = .{ .parts = parts, .span = re.meta.span },
                        .update_source = null,
                        .fields = fields,
                    },
                });
        ...
        for (si.parts) |part| {
            const part_expr: *const ast.Expr = switch (part) {
                ...
                .expr => |expr| blk: {
                    ...
                    // on `value`'s static type. The protocol module
