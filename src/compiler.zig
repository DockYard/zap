...
pub fn compileForCtfe(
    ...
    for (expanded_program.structs) |*mod| {
        const mod_scope = ctx.collector.graph.findStructScope(mod.name) orelse continue;
        for (mod.items) |item| {
            switch (item) {
    ...
    for (desugared_program.structs) |*mod| {
...
pub fn compileModuleByModule(
    ...
    // Shared TypeStore + globally-unique group IDs pipeline.
    {
        ...
        for (module_order, 0..) |mod_name, mod_idx| {
            ...
            // Per-module HIR failure is logged through the diagnostic
            // engine inside the helper; the loop continues so other
            // modules still compile and the user sees as many errors
