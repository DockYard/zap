...
///        local to the share result — under REFCOUNT_V1 a refcount bump
///        returning the same value (identity), under no-REFCOUNT_V1 an
///        independent clone-on-share so each owner reaches a single free.
///        (Was `retainAnyPersistent`, which `shareAnyPersistent` now wraps;
///        the canonical-dispatch count is unchanged.)
