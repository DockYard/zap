pub struct Concurrency.BlockingTest {
  use Zest.Case
  ...
  describe("Process.blocking") {
    test("runs a named i64-returning function off-core and returns its result") {
      answer = Process.blocking(&Concurrency.BlockingTest.answer_entry/0)
      assert(answer == 42)
    }

    test("round-trips a computed result across the off-core detour") {
      product = Process.blocking(&Concurrency.BlockingTest.product_entry/0)
      assert(product == 1806)
    }

    test("round-trips a zero result (the null-pointer boxing edge)") {
      zero = Process.blocking(&Concurrency.BlockingTest.zero_entry/0)
      assert(zero == 0)
    }

    test("round-trips a negative result (the high-bit boxing edge)") {
      negative = Process.blocking(&Concurrency.BlockingTest.negative_entry/0)
      assert(negative == -7)
    }

    test("round-trips a result beyond 32 bits (full i64 width survives boxing)") {
      wide = Process.blocking(&Concurrency.BlockingTest.wide_entry/0)
