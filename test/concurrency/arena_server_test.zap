...
pub struct Concurrency.ArenaServerTest {
  use Zest.Case
  ...
  describe("proven flat server loop — the bounded-heap headline") {
    test("a flat Memory.Arena server's heap stays EXACTLY bounded across 10_000 messages") {
      server = Process.spawn(&Concurrency.ArenaServerTest.flat_server_entry/0, Memory.Arena)
      _channel = Process.send(Process.pid(u64, server), Process.self())

      # Warm up past the first-iteration watermark capture, then sample.
      _warm = Concurrency.ArenaServerTest.send_work(server, 100)
      ...
      _storm = Concurrency.ArenaServerTest.send_work(server, 10000)
  ...
  describe("rejected accumulating loop — the soundness gate holds") {
    test("state retained across iterations survives 1_000 messages (no unsound reset)") {
      server = Process.spawn(&Concurrency.ArenaServerTest.acc_server_entry/0, Memory.Arena)
      _channel = Process.send(Process.pid(u64, server), Process.self())
      ...
      _fill = Concurrency.ArenaServerTest.send_work(server, 1000)
  ...
  describe("the proof is per receive site, not per process") {
    test("one process: an unproven accumulating phase, then a proven flat phase with a bounded heap") {
      server = Process.spawn(&Concurrency.ArenaServerTest.mixed_entry/0, Memory.Arena)
      _channel = Process.send(Process.pid(u64, server), Process.self())

      # Phase 1 (accumulating helper loop — unproven receive): 200 values
      # collected across iterations, reported intact.
      _fill = Concurrency.ArenaServerTest.send_work(server, 200)
      ...
      _storm = Concurrency.ArenaServerTest.send_work(server, 2000)
      _ask2 = Process.send(Process.pid(i64, server), 0)
  ...
  pub fn flat_server_entry() -> Nil {
    ...
    Concurrency.ArenaServerTest.flat_loop(parent)
  }
      ...
      0 ->
        {
          ...
          _work = Concurrency.ArenaServerTest.build_and_sum(1, 50)
      ...
      work ->
        {
          ...
          total = Concurrency.ArenaServerTest.build_and_sum(work, 50)
          _consumed = Concurrency.ArenaServerTest.observe(total)
          Concurrency.ArenaServerTest.flat_loop(parent)
        }
    }
  }
  ...
  pub fn acc_server_entry() -> Nil {
    parent = Process.receive_raw(u64)
    Concurrency.ArenaServerTest.acc_loop(parent, [])
  }
  ...
  pub fn acc_loop(parent :: u64, acc :: List(i64)) -> Nil {
    ...
    case n {
      ...
      0 ->
        {
          checksum = List.length(acc) * 10000000 + Concurrency.ArenaServerTest.sum_list(acc, List.length(acc), 0)
          _reported = Process.send(Process.pid(i64, parent), checksum)
          Concurrency.ArenaServerTest.acc_loop(parent, acc)
        }
      work -> Concurrency.ArenaServerTest.acc_loop(parent, List.push(acc, work))
    }
  }
  ...
  pub fn mixed_entry() -> Nil {
    ...
    checksum = Concurrency.ArenaServerTest.collect_phase([])
    _reported = Process.send(Process.pid(i64, parent), checksum)
  ...
  pub fn collect_phase(acc :: List(i64)) -> i64 {
    ...
    case n {
      0 -> List.length(acc) * 10000000 + Concurrency.ArenaServerTest.sum_list(acc, List.length(acc), 0)
      other -> Concurrency.ArenaServerTest.collect_phase(List.push(acc, other))
    }
  }
  ...
  pub fn send_work(server :: u64, count :: i64) -> Bool {
    Concurrency.ArenaServerTest.send_work_from(server, 1, count)
  }
      ...
      false ->
        {
          ...
          Concurrency.ArenaServerTest.send_work_from(server, next + 1, remaining - 1)
        }
    }
  }
  ...
  pub fn build_and_sum(seed :: i64, length :: i64) -> i64 {
    items = Concurrency.ArenaServerTest.build_list(seed, length, [])
    Concurrency.ArenaServerTest.sum_list(items, List.length(items), 0)
  }

  fn build_list(seed :: i64, remaining :: i64, acc :: List(i64)) -> List(i64) {
    case remaining == 0 {
      true -> acc
      false -> Concurrency.ArenaServerTest.build_list(seed + 1, remaining - 1, List.push(acc, seed))
    }
  }
  ...
  pub fn sum_list(items :: List(i64), index :: i64, acc :: i64) -> i64 {
    case index == 0 {
      true -> acc
      false -> Concurrency.ArenaServerTest.sum_list(items, index - 1, acc + List.get(items, index - 1))
    }
  }
