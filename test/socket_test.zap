pub struct SocketTest {
  ...
  #   * a failed connect surfaces a typed `Socket.Error`, not a crash.
  #
  ...
  describe("Socket loopback and lifetime (gate-OFF)") {
    ...
    test("connect to a closed port yields a typed Socket.Error (not a crash)") {
        ...
        Result.Ok(listener) ->
          {
            ...
            # typed `Result.Error(Socket.Error)`, never an `Ok` or a crash.
  ...
  describe("Socket.set_options / get_option (gate-OFF)") {
    ...
    test("set_options on a stale/closed handle yields a typed Socket.Error, no crash") {
  ...
  fn connect_phase(listener :: Socket.Listener, port :: i64) -> Atom {
    case Socket.connect(Socket.Address.loopback(port), 5000) {
      Result.Error(_e) ->
        {
          _closed = Socket.Listener.close(listener)
  ...
  fn connect_refused?(port :: i64) -> Bool {
    case Socket.connect(Socket.Address.loopback(port), 1000) {
  ...
  fn keepalive_on_client(port :: i64) -> Bool {
    case Socket.connect(Socket.Address.loopback(port), 5000) {
      ...
      Result.Ok(client) ->
        {
          options = %Socket.Options{keepalive: true, recv_buffer: 16384}
  ...
  fn reuse_port_double_bind() -> Bool {
    reuse = %Socket.Options{reuse_port: true, reuse_address: true}
    case Socket.listen(Socket.Address.loopback(0), 8, reuse) {
      Result.Error(_e) -> false
      Result.Ok(first) ->
        {
          port = Socket.Listener.local_port(first)
          second_ok = SocketTest.second_listener_on(port)
          _closed = Socket.Listener.close(first)
          second_ok
        }
    }
  }
  ...
  fn second_listener_on(port :: i64) -> Bool {
    reuse = %Socket.Options{reuse_port: true, reuse_address: true}
    case Socket.listen(Socket.Address.ip4(127, 0, 0, 1, port), 8, reuse) {
      Result.Error(_e) -> false
      Result.Ok(second) ->
        {
          _closed = Socket.Listener.close(second)
          true
        }
    }
  }
