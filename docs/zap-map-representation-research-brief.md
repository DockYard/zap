...
Reminder: the broader plan also involves giving `Vector(T)` a flat-buffer fast path so `MArrayI64` / `MArrayF64` can be deleted. `Vector` already needs a dense representation for that — the agent should consider whether the `Map` and `Vector` redesigns share infrastructure (buffer COW, ownership-aware ABI).
